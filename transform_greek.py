...
def transform_soup_greek(recipe_obj, food_obj_lst, food_name_lst, direc_obj_lst, tools_lst, methods_lst):
	...
	if meat_str:
		step1 = "In a large stock pot, heat vegetable oil on low heat; cook " + meat_str + "until meat is cooked and no longer pink. Remove the meat and dice."
...
def transform_cuisine_main_greek(recipe_obj, food_obj_lst, food_name_lst, direc_obj_lst, tools_lst, methods_lst):
    ...
    #step 3
	step2 = "Spread tomato pesto onto one side of each pita bread and place them pesto-side up on a baking sheet. Top pitas with tomatoes, spinach, mushrooms, feta cheese, Parmesan cheese, and "
    if meat_str:
        step2 += meat_str
    step2pt2 = "; drizzle with olive oil and season with pepper."
	new_direc_obj_lst.append(direction(step2,iing,['spread','top','drizzle','season'],['baking sheet'],[]))
